    ...
    <div class="profile-card">
      <div class="avatar-wrap">
        <div
          v-if="user?.avatar"
          class="avatar"
          :style="{ backgroundImage: `url(${base + user.avatar})` }"
        ></div>
      ...
      <div class="section">
        <h3>Interests</h3>
        <div class="interests">
          <span v-for="i in user?.interests || []" :key="i" class="tag">{{ i }}</span>
          <span v-if="!(user?.interests && user.interests.length)" class="muted"
            >No interests yet</span
          >
        </div>
      </div>

      <div class="actions">
        <button class="btn edit" @click="goEdit">Edit Profile</button>
...
const load = async () => {
  try {
    ...
    console.error('Erro ao carregar perfil:', e.response?.data || e.message)
...
.btn.edit {
  background: linear-gradient(45deg, var(--accent), #b855ff);
  color: #fff;
  min-width: 140px;
}
